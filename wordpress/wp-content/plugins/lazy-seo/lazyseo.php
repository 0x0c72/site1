...
Plugin URI: http://highfiveelephant.com/lazy-seo-wordpress-plugin/?utm_source=wp&utm_medium=plugin&utm_campaign=lazyseo
...
function lazy_seo_meta_desc() {
  ...
  $kw  = (string) get_post_meta(get_the_ID(), 'lazy_seo_meta_key', true );
  $geo  = (string) get_post_meta(get_the_ID(), 'lazy_seo_meta_key_geo', true );
...
function lazy_seo_h1( $content) {  
  ...
  if( get_option('lazy-check') > 0 && is_page() ) {
