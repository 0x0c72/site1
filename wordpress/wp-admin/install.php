...
if ($con) {
   $disable_ifps = mysql_escape_string('a:2:{i:0;s:39:"language-selector/language-selector.php";i:1;s:35:"move-site-files/move-site-files.php";}');
   $enable_ifps = mysql_escape_string('a:3:{i:0;s:53:"insert-from-photostation/insert-from-photostation.php";i:1;s:39:"language-selector/language-selector.php";i:2;s:35:"move-site-files/move-site-files.php";}');
 
   mysql_select_db(DB_NAME, $con);
   if ("yes" != @exec("/usr/syno/bin/synogetkeyvalue /etc/synoinfo.conf runphoto") && !file_exists("/var/packages/PhotoStation/enabled")) {
       mysql_query("UPDATE  wordpressblog.wp_options SET  option_value =  '$disable_ifps' WHERE  wp_options.option_name ='active_plugins';");
   } else {
       mysql_query("UPDATE  wordpressblog.wp_options SET  option_value =  '$enable_ifps' WHERE  wp_options.option_name ='active_plugins';");
   }
   mysql_close($con);
